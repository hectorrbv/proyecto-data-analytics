...
Las variables categóricas se codificaron mediante variables \textit{dummy} con eliminación de la primera categoría para evitar colinealidad. El ajuste se realizó con \texttt{statsmodels}, que permite reportar errores estándar e intervalos de confianza adecuados para inferencia. Para el modelo logístico se utilizó división estratificada del conjunto en entrenamiento (75\,\%) y prueba (25\,\%), con semilla fija para garantizar reproducibilidad y validar el desempeño predictivo fuera de muestra. Los modelos de conteo (Poisson y Binomial Negativa) y el modelo Gamma se ajustaron sobre todo el conjunto, ya que su evaluación se basa en métricas globales de bondad de ajuste (AIC, devianza explicada) y no en predicción individual. La elección de cada métrica responde a la naturaleza de la variable: AUC y curva ROC para la binaria, AIC para los modelos de conteo y devianza explicada para el modelo Gamma.
